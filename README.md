...
本仓库汇集了从入门到实战的多类爬虫脚本与案例，覆盖 requests/正则/并发/selenium/CDP 抓包、文件下载与解析等能力，同时包含一个可直接使用的“小红书 Selenium 集成方案”。
...
- `抖音小红书/小红书爬取/`：
  - `小红书Selenium集成方案.py` 与说明/修改历史文档
  - `小红书爬取关键词笔记.py` 与结构说明文档
  - 关键词搜索、登录态保持、CDP 抓包、详情页解析、图片/视频并发下载
...
- 小红书集成方案：`抖音小红书/小红书爬取/小红书Selenium集成方案.py`
...
- **`抖音小红书/小红书爬取/`**：
  - `小红书Selenium集成方案.py`（主程序，集成登录保持+抓包+批量抓取）。
  - `小红书Selenium集成方案_修改历史.md`（变更记录）。
...
- **延伸阅读**：
  ...
  - 变更记录：`抖音小红书/小红书爬取/小红书Selenium集成方案_修改历史.md`。
...
- **隐私与大文件**：请在 `.gitignore` 忽略 `chrome_user_data/` 与大体量下载产物。
...
- **隐私保护**：不提交任何个人 Cookie、账号、令牌、浏览器用户数据；
  - 将 `chrome_user_data/` 加入 `.gitignore`；
  - 不提交下载所得的大体量媒体文件与敏感样本。
